...
def platformStats(request):
    ...
    from scholaprefs.models import UserPreferenceSession
    return Response({
        'users': User.objects.count(),
        'schools': School.objects.count(),
        'comments': Comment.objects.count(),
        'tests': SessionTest.objects.count() + UserPreferenceSession.objects.count(),
    })
...
def update_profile(request):
    ...
    user.prenom = request.data.get('prenom', request.data.get('first_name', user.prenom))
    user.nom = request.data.get('nom', request.data.get('last_name', user.nom))
...
from .models import PersonalityTest, TestQuestion, Choice, SessionTest, ResponseTest
...
@permission_classes([])
@authentication_classes([])
def get_personality_test(request):
    try:
        # Fetch the first active test
        test = PersonalityTest.objects.first()
        if not test:
            return Response({'error': 'No personality test found'}, status=404)
        serializer = PersonalityTestSerializer(test)
        return Response(serializer.data)
    except Exception as e:
        return Response({'error': str(e)}, status=500)
...
def submit_test(request):
    try:
        ...
        # User auth check - if authenticated, use their id, else use 0 for guest
        token = request.headers.get('Authorization', '').replace('Token ', '')
        user_id = 0
        ...
        session = SessionTest.objects.create(
        ...
        for resp in responses_data:
            ...
            c_id = resp.get('choice_id')
            if q_id and c_id:
                try:
                    question = TestQuestion.objects.get(id_question=q_id)
                    choice = Choice.objects.get(id_choice=c_id)
                    ResponseTest.objects.create(
                        id_session=session,
                        id_question=question,
                        id_choice=choice
                    )
                except (TestQuestion.DoesNotExist, Choice.DoesNotExist):
                    continue

        return Response({'success': True, 'session_id': session.id_session})
    except Exception as e:
        return Response({'error': str(e)}, status=500)
