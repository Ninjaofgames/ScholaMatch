...
def extract_aspects_with_polarity(review_text: str) -> list[dict]:
    """
    Extract aspects and their polarity from a review text.
    Returns a list like:
    [{"aspect": "teachers", "polarity": "positive"}, ...]
    """
    if not review_text or not review_text.strip():
        return []

    prompt = (
        'Extract all aspects mentioned in this school review and their sentiment.\n'
        'Return ONLY valid JSON with the key "aspects".\n'
        'Each item must have "aspect" (short noun) and "polarity" (positive/negative/neutral).\n\n'
        'Expected format: {"aspects": [{"aspect": "teachers", "polarity": "positive"}, ...]}\n\n'
        f'Review:\n"{review_text}"\n'
    )
    ...
    try:
        ...
        return [
            {'aspect': a['aspect'].strip().lower(), 'polarity': a['polarity'].strip().lower()}
            for a in aspects
            if 'aspect' in a and 'polarity' in a
            and a['polarity'].strip().lower() in ['positive', 'neutral', 'negative']
        ]
    except Exception as exc:
        logger.error('Aspect extraction error: %s', exc)
        return []

def classify_aspect_category(aspect_name: str) -> str:
    """
    Classify a free-form aspect name into one of 4 categories:
    teachers, facilities, administration, affordability.
    Falls back to 'other' if not classifiable.
    """
    prompt = (
        'Classify the following school aspect into exactly one of these categories:\n'
        '- teachers (teaching quality, professors, pedagogy)\n'
        '- facilities (campus, buildings, equipment, classrooms, labs)\n'
        '- administration (management, staff, organization, bureaucracy)\n'
        '- affordability (price, cost, tuition, fees, money)\n\n'
        'Rules:\n'
        '- Return ONLY valid JSON with key "category"\n'
        '- Value must be exactly one of: teachers, facilities, administration, affordability, other\n\n'
        'Expected format: {"category": "teachers"}\n\n'
    ...
    try:
        ...
        if category in ('teachers', 'facilities', 'administration', 'affordability'):
            return category
        return 'other'
    except Exception as exc:
        logger.error('Aspect category classification error: %s', exc)
        return 'other'