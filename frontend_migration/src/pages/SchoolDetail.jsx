...
const SchoolDetail = () => {
  ...
  useEffect(() => {
    const fetchSchoolData = async () => {
      try {
        ...
        const detailRes = await fetch(`/api/schools/${id}/`);
        if (detailRes.ok) {
        ...
        // Fetch comments
        const commentsRes = await fetch(`/api/schools/${id}/comments/`);
        if (commentsRes.ok) {
  ...
  const handleCommentSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`/api/schools/${id}/comments/`, {
        method: 'POST',
        headers: {
